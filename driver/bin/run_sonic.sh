...
duration=5
mac_wait=3
mode=pcs_app_cross,pcs_app_cross
...
function run_sonic {
    echo tester mode=$mode pkt_len=$pkt_len duration=$duration idle=$idle pkt_cnt=$pkt_cnt wait=$mac_wait mac_src=$mac_src mac_dst=$mac_dst ip_src=$ip_src ip_dst=$ip_dst port_src=$port_src port_dst=$port_dst vlan_id=$vlan_id gen_mode=$gen_mode delta=$epsilon chain_idle=$chain_idle chain_num=$chain_num> /proc/sonic 
...
function pre_process {
    replay_path=$1
    $SONIC_DIR/examples/sonic_rpt 0 $replay_path
}

function post_process {
    result_path=$1
    $SONIC_DIR/examples/sonic_cap 1 $result_path
    #FIXME
...
do
    ...
    -)
        case "${OPTARG}" in
            ...
            ;;
            chain_idle) chain_idle=${!OPTIND}
            ;;
            chain_num) chain_num=${!OPTIND}
            ;;
        esac
        OPTIND=$((OPTIND + 1))
        ;;
