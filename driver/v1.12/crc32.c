...
uint32_t crc32_le2(uint32_t crc, unsigned char const *p, size_t len)
{
	int i;
	while (len--) {
		crc ^= *p++;
...
static void store_be(uint32_t x, unsigned char *buf)
{
 ...
 */
static uint32_t test_step(uint32_t init, unsigned char *buf, size_t len)
{
	uint32_t crc1, crc2;
	size_t i;
