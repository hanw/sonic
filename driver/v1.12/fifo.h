...
struct sonic_fifo {
	...
	int reserved3[15];
//	int reserved;
	struct sonic_fifo_entry entry[FIFO_NUM_ENTRY];
};
...
//	while (__sync_and_and_fetch(flag, 1) > 0)
...
// In-efficient FIFO implementation
#if 0	 
static inline void * get_write_entry(struct sonic_fifo *fifo)
{
	return ALLOC_PAGES2(fifo->porder);	
}
...
//	while (__sync_and_and_fetch(flag, 1) > 0)
	...
	fifo->entry[fifo->w_tail].data = data;
...
//	while (__sync_and_and_fetch(flag, 1) == 0)
	...
	ret = fifo->entry[fifo->r_head].data;
...
int fifo_entry_size( struct sonic_fifo *);
