...
//        int *eof, void *data)
static ssize_t sonic_proc_read(struct file *file, char __user *buf, size_t count,
        loff_t *eof)
{
...
static ssize_t sonic_proc_write(struct file *filp, const char __user *buff, 
...
static struct file_operations sonic_proc_fops = {
	.read = sonic_proc_read,
	.write = sonic_proc_write,
};
...
int sonic_proc_init(struct sonic_priv * sonic)
{
    ...
    * (char *) sonic->procpage = '\0';
    ...
    if (!(sonic->procentry = proc_create("sonic", 0666, 
            NULL, &sonic_proc_fops))) {
