...
static void sonic_set_fifo_pcs_cross_forwarding(struct sonic_port *port) {
    struct sonic_priv *sonic = port->sonic;
    struct sonic_fifo *fifo1 = port->fifo[0];

    int other_port_id = port->port_id ? 0 : 1;
    struct sonic_port * other_port = sonic->ports[other_port_id];

    port->rx_pcs->out_fifo = fifo1;
    other_port->tx_pcs->in_fifo = fifo1;
}

static void sonic_set_fifo_pcs_app_cross_forwarding(struct sonic_port *port ){
    struct sonic_priv *sonic = port->sonic;
    struct sonic_fifo *fifo1 = port->fifo[0];
    struct sonic_fifo *fifo2 = port->fifo[1];
    ...
    port->app->out_fifo = fifo2;
    other_port->tx_pcs->in_fifo = fifo2;
}
...
static struct sonic_runtime_thread_funcs pcs_cross= {
    .rx_pcs = sonic_pcs_rx_loop,
    .tx_pcs = sonic_pcs_tx_loop,
    .set_fifo = sonic_set_fifo_pcs_cross_forwarding,
};

static struct sonic_runtime_thread_funcs pcs_app_cross= {
    .rx_pcs = sonic_pcs_rx_loop,
    .app = sonic_app_availbw_loop,
    .tx_pcs = sonic_pcs_tx_loop,
    .set_fifo = sonic_set_fifo_pcs_app_cross_forwarding,
};
...
#define SONIC_THREADS(x,y,z)   \
        port->x->cpu = sargs->port_socket[i] + ((random++)%(total_cpu_socket-1)+1)*2;
        SONIC_THREADS_ARGS
#undef SONIC_THREADS
