...
ssh -i ~/.ssh/id_rsa2 $SONIC_SERVER "$SONIC_PATH/bin/run_sonic.sh -m pkt_gen,pkt_cap -d 35 -w 32 --mac_src $mac_src --ip_src $ip_src --port_src $port_src --mac_dst $mac_dst --ip_dst $ip_dst --port_dst $port_dst -c $pkt_cnt -l $pkt_len -i $idle -e $epsilon -g 3 -p $SONIC_PATH/result/${prefix}_covert_${epsilon} -r $SONIC_PATH/bin/tmp.info"
